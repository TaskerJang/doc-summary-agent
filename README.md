...
**기업공시·사업보고서·산업분석 리포트를 파싱·요약·질의응답하는 AI 에이전트**
...
[![uv](https://img.shields.io/badge/packaged_with-uv-261230)](https://github.com/astral-sh/uv)
...
<table>
  <tr>
    <td align="center"><img src="docs/images/01-login.jpeg" width="280"/><br/><sub><b>🔐 Password 로그인</b></sub></td>
    <td align="center"><img src="docs/images/02-summary.jpeg" width="280"/><br/><sub><b>📝 문서 업로드 후 전체 요약</b></sub></td>
    <td align="center"><img src="docs/images/03-charts.jpeg" width="280"/><br/><sub><b>📊 섹션별 자동 차트</b></sub></td>
  </tr>
</table>
...
|  |  |
|---|---|
| 📑 **다중 포맷 파싱** | PDF / DOCX / DOC / HWP + 이미지 기반 PDF는 EasyOCR(dpi=300)로 자동 처리 |
| 🧹 **노이즈 제거 전처리** | 헤더·푸터·면책고지 등 자동 필터링 |
| ✂️ **Markdown 구조 청킹** | 섹션/단락/표 단위로 분할 |
| 📝 **Map-Reduce 요약** | GPT-5.2 기반 청크 병렬 요약 → 전체 요약 |
| 🔍 **하이브리드 검색 Q&A** | BM25(sparse) + bge-m3(dense) + RRF 융합 + bge-reranker-v2-m3 재정렬 |
| 📊 **섹션별 자동 차트** | 수치 데이터를 Plotly로 자동 렌더링 (on/off · 개수 설정 가능) |
| 💡 **추천 질문 버튼** | 요약 결과 기반으로 후속 질문 3개 자동 생성 |
...
| 🎨 **ChatGPT 스타일 UI** | Chainlit 기반 · 스트리밍 · 소스 인용 · TaskList 진행 표시 |
| 🧪 **평가 파이프라인** | ROUGE · 수치 정확도 · Faithfulness · Completeness 자동 측정 |
...
> 평가 시점은 BM25 단독 시절 — 하이브리드 검색 도입 이후 재측정 예정.
...
> **Windows**
> - CMD: `set APP_USERNAME=admin` / `set APP_PASSWORD=your-password`
> - PowerShell: `$env:APP_USERNAME = "admin"`
...
- **현재 한계**: 과거 thread 재진입 시 차트/PDF는 복원되지 않는다 (blob_storage 미설정). 자세한 내용은 이슈 [#101](https://github.com/TaskerJang/doc-summary-agent/issues/101) 참고.
- **resume 시 세션 상태 복원**(`@cl.on_chat_resume`)은 이슈 [#99](https://github.com/TaskerJang/doc-summary-agent/issues/99)에서 후속 작업 중.
...
<div align="center">
<sub>Made with ☕ & 🐍 · Pinetree Partners Intern Project</sub>
</div>
